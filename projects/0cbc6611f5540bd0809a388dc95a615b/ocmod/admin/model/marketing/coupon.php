...
class ModelMarketingCoupon extends Model {
    ...
    public function createCouponGiftProductTable() {
        $this->db->query("
		CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "coupon_gift_product` (
    ...
    public function addCoupon($data) {
        ...
        if (isset($data['coupon_gift'])) {
            foreach ($data['coupon_gift'] as $product_id) {
    ...
    public function getCouponGiftsProducts($coupon_id) {
        $coupon_gift_products_data = array();

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_gift_product WHERE coupon_id = '" . (int)$coupon_id . "'");

        foreach ($query->rows as $result) {
            $coupon_gift_products_data[] = $result['product_id'];
        }

        return $coupon_gift_products_data;
    }
